...
# Create a new branch which we want to migrate to, with a new content iteration
os_repository_new_commit 1 1 testos/buildmaster/newbranch
# bootcsum now refers to this new commit

# Mark the old branch as EOL and delete it, and create the summary file
echo "[endoflife]" >> ${test_tmpdir}/testos-repo/config
echo "testos/buildmaster/newbranch=testos/buildmaster/x86_64-runtime" >> ${test_tmpdir}/testos-repo/config
${CMD_PREFIX} ostree --repo=testos-repo refs --delete testos/buildmaster/x86_64-runtime
${CMD_PREFIX} ostree --repo=testos-repo summary -u
