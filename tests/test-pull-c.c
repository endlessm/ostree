...
test_pull_multi_nochange (gconstpointer data)
{
  ...
  const char *refs[] = { "main", NULL };

  if (!ostree_repo_pull (td->repo, "origin", refs, 0, NULL, NULL, error))
    goto out;
  if (!ostree_repo_pull (td->repo, "origin", refs, 0, NULL, NULL, error))
    goto out;
  if (!ostree_repo_pull (td->repo, "origin", refs, 0, NULL, NULL, error))
...
test_pull_multi_error_then_ok (gconstpointer data)
{
  ...
  TestData *td = (void*)data;
  const char *ok_refs[] = { "main", NULL };
  const char *bad_refs[] = { "nosuchbranch", NULL };

  for (guint i = 0; i < 3; i++)
    {
      g_autoptr(GError) tmp_error = NULL;
      if (!ostree_repo_pull (td->repo, "origin", ok_refs, 0, NULL, NULL, error))
        goto out;
      if (ostree_repo_pull (td->repo, "origin", bad_refs, 0, NULL, NULL, &tmp_error))
