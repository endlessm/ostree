 ...
 */
static gboolean
ostree_repo_pull_with_options_internal (OstreeRepo *self, const char *remote_name_or_baseurl,
                               ...
                               GCancellable *cancellable, GError **error)
{
  ...
  if (!ostree_repo_pull_with_options_internal (self, remote_name_or_baseurl, options, progress,
                                               cancellable, &local_error))
    {
      ...
      if (g_error_matches (local_error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND) && !disable_static_deltas
          && !require_static_deltas)
        {
          ...
          g_variant_dict_insert (&modified_options_dict, "disable-static-deltas", "b", TRUE);
          modified_options = g_variant_dict_end (&modified_options_dict);

          return ostree_repo_pull_with_options_internal (
              self, remote_name_or_baseurl, modified_options, progress, cancellable, error);
        }
