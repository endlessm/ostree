  ...
  for (guint i = 0; i < n; i++)
    {
      ...
      enqueue_one_object_request (pull_data, file_checksum, OSTREE_OBJECT_TYPE_FILE, path, FALSE, OSTREE_FETCH_OBJECT_CORE, NULL);
                        ...
                        gpointer           user_data)
{
  ...
  ostree_object_name_deserialize (fetch_data->object, &checksum, &objtype);
  checksum_obj = ostree_object_to_string (checksum, objtype);
  g_debug ("fetch of %s%s%s%s complete", checksum_obj,
          ...
          if (fetch_data->type != OSTREE_FETCH_OBJECT_CORE)
            {
              ...
              if (next == OSTREE_FETCH_OBJECT_CORE && fetch_data->object_is_stored)
                ...
                enqueue_one_object_request (pull_data, checksum, objtype, fetch_data->path, next,
                                            fetch_data->object_is_stored, fetch_data->requested_ref);
            }
              ...
              if (pull_data->has_tombstone_commits)
                {
                  enqueue_one_object_request (pull_data, checksum, OSTREE_OBJECT_TYPE_TOMBSTONE_COMMIT,
                                              fetch_data->path, OSTREE_FETCH_OBJECT_CORE, FALSE, NULL);
                }
            }
        }
  ...
  if (fetch_data->type == OSTREE_FETCH_OBJECT_DETACHED_METADATA)
  ...
  else if (fetch_data->type == OSTREE_FETCH_OBJECT_COMPAT_SIGNATURE)
    {
      ...
      if (!fetch_data->object_is_stored)
        enqueue_one_object_request (pull_data, checksum, objtype, fetch_data->path,
                                    OSTREE_FETCH_OBJECT_CORE, FALSE, fetch_data->requested_ref);
      else
        queue_scan_one_metadata_object (pull_data, checksum, objtype, fetch_data->path, 0, fetch_data->requested_ref);
    }
  else
  ...
  if (!is_stored && !is_requested)
    {
      ...
      enqueue_one_object_request (pull_data, tmp_checksum, objtype, path, fetchtype, FALSE, ref);
    }
  else if (is_stored && objtype == OSTREE_OBJECT_TYPE_COMMIT)
    {
      ...
      if (!g_hash_table_contains (pull_data->fetched_detached_metadata, tmp_checksum))
        enqueue_one_object_request (pull_data, tmp_checksum, objtype, path,
                                    OSTREE_FETCH_OBJECT_DETACHED_METADATA, TRUE, ref);
      else
        {
             ...
             FetchObjectData *fetch)
{
  ...
  if (fetchtype == OSTREE_FETCH_OBJECT_DETACHED_METADATA)
    ...
    {
      char buf[_OSTREE_LOOSE_PATH_MAX];
      _ostree_loose_path_with_extension (buf, expected_checksum, "sizes2");
      obj_subpath = g_build_filename ("objects", buf, NULL);
      mirrorlist = pull_data->meta_mirrorlist;
    }
    ...
    {
      char buf[_OSTREE_LOOSE_PATH_MAX];
      _ostree_loose_path_with_extension (buf, expected_checksum, "sig");
      obj_subpath = g_build_filename ("objects", buf, NULL);
      mirrorlist = pull_data->meta_mirrorlist;
    }
  else
   ...
   */
  expected_max_size_p = (fetchtype != OSTREE_FETCH_OBJECT_CORE) ? NULL : g_hash_table_lookup (pull_data->expected_commit_sizes, expected_checksum);
  if (expected_max_size_p)
    expected_max_size = *expected_max_size_p;
  else if (OSTREE_OBJECT_TYPE_IS_META (objtype) && fetchtype != OSTREE_FETCH_OBJECT_COMPAT_SIZES)
          ...
          if (!g_hash_table_lookup (pull_data->requested_content, checksum))
            {
              ...
              enqueue_one_object_request (pull_data, checksum, OSTREE_OBJECT_TYPE_FILE, NULL,
                                          OSTREE_FETCH_OBJECT_CORE, FALSE, NULL);
  ...
  if (pull_data->summary)
    {
      ...
      if (delta_from_revision)   /* Did we find a delta FROM commit? */
        enqueue_one_static_delta_superblock_request (pull_data, delta_from_revision, to_revision, ref);
