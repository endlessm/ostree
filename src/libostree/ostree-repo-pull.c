  ...
  if (!temp_path)
    {
      if (!g_error_matches (local_error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND))
        goto out;
      else if (fetch_data->type != OSTREE_FETCH_OBJECT_CORE)
        {
          ...
          enqueue_one_object_request (pull_data, checksum, objtype, next);
        }
  ...
  if (!is_stored && !is_requested)
    {
      ...
      enqueue_one_object_request (pull_data, tmp_checksum, objtype, fetchtype);
    }
  else if (is_stored)
    {
