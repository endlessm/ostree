...
static gboolean
repo_should_store_sizes (OstreeRepo *self)
{
  return ostree_repo_get_mode (self) == OSTREE_REPO_MODE_ARCHIVE &&
         self->generate_sizes;
}
                       ...
                       GError            **error)
{
  ...
  if (is_tombstone)
  ...
  else
    {
      ...
      if (have_obj)
        {
          if (repo_should_store_sizes (self))
            repo_store_size_entry (self, objtype, actual_checksum,
                                   g_bytes_get_size (buf), g_bytes_get_size (buf));
  ...
  if (expected_checksum)
    {
      ...
      if (!_ostree_repo_has_loose_object (self, expected_checksum, objtype, &have_obj,
                                          cancellable, error))
        return FALSE;
      if (have_obj && !repo_should_store_sizes (self))
        {
          if (out_csum)
                           ...
                           GError          **error)
{
   ...
   *
   * If we’re writing out size metadata, fall through to the write_content_object()
   * call anyway, since we need all the objects to end up in self->object_sizes.
   */
  if (expected_checksum)
    {
      gboolean have_obj;
