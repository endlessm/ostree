                               ...
                               GError    **error)
{
  ...
  gs_unref_object OtDeployment *old_deployment = NULL;
  ...
  if (!ot_admin_list_deployments (sysroot,
                                  &cur_bootversion, &cur_deployments,
                                  NULL, error))
    goto out;

  if (!ot_admin_require_deployment_or_osname (sysroot, cur_deployments,
                                              NULL, &old_deployment,
                                              NULL, error))
    goto out;

  osname = ot_deployment_get_osname (old_deployment);
  merge_deployment =
    ot_admin_get_merge_deployment (cur_deployments, osname, old_deployment);
  origin = ot_deployment_get_origin (merge_deployment);
  booted = ot_deployment_get_csum (merge_deployment);
