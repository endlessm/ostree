...
gboolean
ostree_builtin_size_summary (int argc, char **argv, OstreeCommandInvocation *invocation, GCancellable *cancellable, GError **error)
{
  ...
  if (!ostree_option_context_parse (context, NULL, &argc, &argv, invocation, &repo, cancellable, error))
    return FALSE;
